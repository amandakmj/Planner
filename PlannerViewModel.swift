...
class PlannerViewModel: ObservableObject {
    ...
    init() {
        // ⚠️ Isso limpa os dados salvos para forçar carregar o mock atualizado
        UserDefaults.standard.removeObject(forKey: aulasKey)
        UserDefaults.standard.removeObject(forKey: forcadasKey)
        carregarAulas()
    }
    ...
    private let aulasKey = "aulas_salvas"
    private let forcadasKey = "aulas_forcadas"
    ...
    private func carregarAulas() {
        if let data = UserDefaults.standard.data(forKey: aulasKey),
           let decoded = try? JSONDecoder().decode([Aula].self, from: data) {
            ...
            self.aulas = AulasMock.lista // Fallback inicial
        }
